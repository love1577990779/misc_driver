...
static void __fill_v4l2_buffer(struct vb2_buffer *vb, void *pb)
{
	...
	struct vb2_queue *q = vb->vb2_queue;
	unsigned int plane;
	printk(KERN_ALERT "__fill_v4l2_buffer enter!\r\n");
	...
	printk(KERN_EMERG "b->m.offset = 0x%x\r\n" , b->m.offset);
	/*
...
int vb2_querybuf(struct vb2_queue *q, struct v4l2_buffer *b)
{
	...
	vb = q->bufs[b->index];
	printk(KERN_ALERT "vb = 0x%x , q = 0x%x , index = %d\r\n" , vb , q , b->index);
	ret = __verify_planes_array(vb, b);
	if (!ret)
...
int vb2_reqbufs(struct vb2_queue *q, struct v4l2_requestbuffers *req)
{
	...
	printk(KERN_ALERT "q->bufs[0] = 0x%x , q = 0x%x\r\n" , q->bufs[0] , q);
}
...
{
	printk(KERN_ALERT "vb2_ioctl_reqbufs : enter!\r\n");
