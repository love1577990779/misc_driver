...
static int __vb2_buf_mem_alloc(struct vb2_buffer *vb)
{
	...
	printk(KERN_EMERG "__vb2_buf_mem_alloc enter!\r\n");
	for (plane = 0; plane < vb->num_planes; ++plane) {
		...
		printk(KERN_EMERG "call_ptr_memop enter!\r\n");
			     ...
			     const unsigned plane_sizes[VB2_MAX_PLANES])
{
	...
	for (buffer = 0; buffer < num_buffers; ++buffer) {
		...
		printk(KERN_ALERT "q = 0x%x , vb = 0x%x\r\n" , q , vb);
		if (!vb) {
		...
		if (memory == VB2_MEMORY_MMAP) {
			...
			printk(KERN_EMERG "__vb2_buf_mem_alloc ret = %d\r\n" , ret);
			if (ret) {
...
int vb2_core_reqbufs(struct vb2_queue *q, enum vb2_memory memory,
		     unsigned int *count)
{
	printk(KERN_ALERT "vb2_core_reqbufs : enter\r\n");
	...
	printk(KERN_ALERT "CALL __vb2_queue_alloc\r\n");
