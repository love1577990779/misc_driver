				   ...
				   int state)
{
	...
	if (type == EV_REL) {
		...
		printk(KERN_ALERT "type = %d , button->code = %d , state = %d\r\n" , type , button->code , state);
...
{
	printk(KERN_ALERT "gpio_keys_polled_probe\r\n");
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (!dev_get_platdata(dev)) {
			 ...
			 */
			printk(KERN_ALERT "gpio_is_valid\r\n");
...
static struct gpio_keys_button simple_buttons[] = {
    [0] = {
        .code = KEY_L,
        .gpio = 400,
        .desc = "S2",
...
static struct gpio_keys_platform_data simple_keys_pdata = {
    .buttons = simple_buttons,
    .nbuttons = ARRAY_SIZE(simple_buttons),
	.poll_interval = 100,
};
...
static struct platform_device simple_gpio_keys_polled_device = {
	.name = DRV_NAME,
    .dev = {
        .platform_data = &simple_keys_pdata,
    },
};
...
static int __init simple_gpio_keys_polled_init(void) 
{
	platform_device_register(&simple_gpio_keys_polled_device);
	platform_driver_register(&gpio_keys_polled_driver);
	return 0;
}

static void __exit simple_gpio_keys_polled_exit(void) 
{
	platform_driver_unregister(&gpio_keys_polled_driver);
	platform_device_unregister(&simple_gpio_keys_polled_device);
}


module_init(simple_gpio_keys_polled_init);
module_exit(simple_gpio_keys_polled_exit);
